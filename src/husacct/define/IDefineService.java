...
public interface IDefineService extends ISaveable, IObservableService {
	 ...
	 * @param projects:
	 *            is an ArrayList<ProjectDTO> with all the projects within the application
	 * @param version:
	 ...
	 * 
	 * @param String
	 *            logicalPath is the logical path, ** is root module
	 ...
	 * 
	 * @param physicalPath
	 *            is the physical path of a class or interface
	...
	/** Adds a module to the intended architecture Argument name is the simple name, so not the logical path of the module, which is formed by the
	 * combination of parentLogicalPath + "." + name. */
	...
	/** Edits an existing module of the intended architecture Argument logicalPath identifies the existing module. If the other arguments don't have a
	 * null-value (or 0 for int), the value is set as new attribute value. Argument name is the simple name, so not the logical path of the module. */
